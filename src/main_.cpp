...
int loadWeights(Weights &weights, const char *safetensors_path)
{
    if (checkGPUStatus() != 0)
    {
        return 1;
    ...
    {
        std::cout << "Can't open " << safetensors_path << " file\n";
    ...
    for (int i = 0; i < N_LAYERS; ++i)
    {
        ...
        weights.k_proj[i] = (__nv_bfloat16 *)((char *)model_weights + offsets.at("model.layers." + std::to_string(i) + ".self_attn.k_proj.weight"));
...
int main(int argc, char *argv[])
{
    ...
    Weights weights{};
    const char *safetensors_path = argc > 1 ? argv[1] : "model.safetensors";
    if (loadWeights(weights, safetensors_path) != 0)
    {
        return 1;
    }
